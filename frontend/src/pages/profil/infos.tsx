...
import { DELETE_VEHICLE, GET_USER_PROFILE_INFOS } from "@/graphql/client";

const ProfilPage = () => {
  const [userInfos, setUserInfos] = useState<UserUpdate>();
  const { refetch } = useQuery(GET_USER_PROFILE_INFOS, {
    onCompleted: (data) => {
  ...
  const [deleteCard] = useMutation(DELETE_VEHICLE, {
    variables: {
      ...
      <div className="flex flex-col w-full mt-8 gap-10">
        <div className="flex flex-row justify-center gap-40 items-center">
          ...
          {userInfos?.picture ? (
            <img
              ...
              className="w-36 h-36 rounded-full object-contain border-double border-4 border-emerald-600"
            />
          ) : (
            <Image
              src={DefaultProfilPicture.src}
              alt="default profil picture"
        ...
        <div className="flex flex-col gap-10 text-center">
          ...
          <div className="flex justify-center gap-20 text-left">
            <span>
              Date de naissance:{" "}
              {formatBirthDate(userInfos?.birthday as string)}
            </span>
        ...
        {userInfos?.vehicle ? (
          <>
            ...
            <div className="flex flex-col md:flex-row justify-center items-center gap-4 mt-6">
              <Button
                ...
                radius="full"
                onClick={() =>
                  router.push(`/profil/updatecar/${userInfos?.vehicle?.id}`)
                }
              >
          ...
          <>
            <h3 className="text-center">
              Vous n'avez pas de véhicule renseigné sur votre profil.
            </h3>
            <Button
              type="submit"
              color="primary"
