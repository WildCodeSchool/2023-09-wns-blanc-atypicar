            ...
            <div className="flex flex-col justify-between items-center p-2 h-52">
                <div className="flex flex-col md:flex-row">
                    <CardHeader className="grid grid-cols-[60px_20px_minmax(0,_1fr)] justify-items-start font-bold font-montserrat w-52">
                        <p>{formatHour(reservation.journey.startDate)}</p>
                        <div className="w-3 h-3 border-solid border-2 border-black rounded-full"></div>
                        <h4>{reservation.journey.startingPoint}</h4>

                        <small className="pl-2 text-default-500 font-normal">
                            {calculateDuration(reservation.journey.startDate, reservation.journey.endDate)}
                        </small>
                        <div className="h-9 -mb-1.5 -mt-2 ml-1 w-1 border-solid border-2 border-black "></div>
                        <div></div>

                        <p>{formatHour(reservation.journey.endDate)}</p>
                        <div className="w-3 h-3 border-solid border-2 border-black rounded-full"></div>
                        <h4>{reservation.journey.arrivalPoint}</h4>
                    </CardHeader>
                    <CardBody className="py-1 ml-0 md:ml-4 md:p-3 w-auto">
                        <button className={`px-4 py-1 text-white rounded-full ${reservation.status === 'VALIDATED' ? 'bg-green-500' : reservation.status === 'CANCELED' ? 'bg-red-500' : ''}`}>
                            {reservation.status === 'VALIDATED' ? 'Validé' : reservation.status === 'CANCELED' ? 'Refusé' : reservation.status}
                        </button>
                ...
                <CardFooter className="flex justify-start gap-6 items-center w-full pt-2">
                    ...
                    />
                    <h4>username</h4>
                    <h4 className="ml-auto">{reservation.journey.price}€</h4>
                </CardFooter>
            </div >
        </Card >
    );
