...
export default function SearchBar({ startingPoint, endingPoint, dateStart, availableSeats }) {
    const [start, setStart] = useState<string>(startingPoint ? startingPoint : "");
    const [end, setEnd] = useState<string>(endingPoint ? endingPoint : "");
    const [date, setDate] = useState<string>(dateStart ? dateStart : "");
    const [seats, setSeats] = useState<string>(availableSeats ? availableSeats : "");
    ...
    return (
        <div className="flex flex-col sm:flex-row items-center justify-center  mt-28 lg:mt-54   ">
            ...
            <Input placeholder="Destination" classNames={{
            ...
            }} className="sm:w-64 w-80 shadow-lg  " radius="none"
                startContent={
                    <CgShapeCircle className="text-2xl  pointer-events-none flex-shrink-0 text-secondary" />
                } value={end} onChange={(e) => setEnd(e.target.value)} />
            <div className="flex flex-row ">
                <Input type="datetime-local" classNames={{
                    inputWrapper: "bg-white after:content-[''] after:absolute after:h-10 after:right-0 after:w-1 after:bg-secondary after:rounded",
                }} className="sm:w-52 w-48 shadow-lg " radius="none" startContent={
                    <IoCalendarOutline className="text-2xl  pointer-events-none flex-shrink-0 text-secondary" />
                } min={(new Date()).toISOString().split("T")[0]} value={date} onChange={(e) => setDate(e.target.value)} />
                <Input type="number" classNames={{
                    inputWrapper: "bg-white    ",
                }} placeholder="Passager" className="sm:w-36 w-32 shadow-lg " radius="none" startContent={
                    <IoPersonOutline className="text-2xl  pointer-events-none flex-shrink-0 text-secondary" />
                } min="0" max="20" step="1" value={seats} onChange={(e) => setSeats(e.target.value)} />
            </div>

            <button className="bg-success text-white  w-80 sm:w-32 shadow-lg  rounded-b-xl sm:rounded-bl-none sm:rounded-r-xl p-4" onClick={search}>Rechercher</button>
        </div>
    )
}
