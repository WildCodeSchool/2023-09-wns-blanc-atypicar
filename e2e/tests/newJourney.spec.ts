...
test.beforeEach(async ({ page }) => {
  await page.goto("http://localhost:3000/journeys/new");
});
...
test("should add a new journey and redirect to journeys page", async ({
  ...
  await page.waitForURL("http://localhost:3000/journeys");

  expect(page.url()).toEqual("http://localhost:3000/journeys");
});
